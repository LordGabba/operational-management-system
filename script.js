...
function normalizarFormularioColaborador(dados) {
  const normalizado = { ...dados };

  ['admissao', 'data_nasc', 'primeiro_dia_ferias', 'ultimo_dia_ferias', 'data_desligamento'].forEach(campo => {
    if (normalizado[campo] === '') normalizado[campo] = null;
  });

  ['banco_horas', 'jornada_semanal'].forEach(campo => {
    if (normalizado[campo] === '') {
      normalizado[campo] = null;
    } else if (normalizado[campo] !== undefined && normalizado[campo] !== null) {
      const n = Number(String(normalizado[campo]).replace(',', '.'));
      normalizado[campo] = Number.isFinite(n) ? n : null;
    }
  });
...
async function salvarColaborador() {
  const form = document.getElementById('form-colaborador');
  if (!form) return;
  const dados = normalizarFormularioColaborador(Object.fromEntries(new FormData(form)));
  // Validações
...
function parsearCSV(texto) {
  ...
  const limparValor = (v) => String(v ?? '').trim().replace(/^"|"$/g, '');

  const headers = primeiraLinha.split(separador).map(limparValor);

  return linhas.slice(1).map(linha => {
    const valores = linha.split(separador).map(limparValor);
    const obj = {};
    headers.forEach((h, i) => {
      obj[h] = valores[i] || '';
    });
...
async function confirmarImportacao(dados) {
  ...
  const normalizarChave = (v) => limpar(v)
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    .replace(/[^a-z0-9]/g, '');
  ...
  const get = (obj, nomes) => {
    ...
    Object.keys(obj).forEach(k => {
      mapa[normalizarChave(k)] = obj[k];
    });

    for (const nome of nomes) {
      const valor = mapa[normalizarChave(nome)];
      if (valor !== undefined && limpar(valor) !== '') return limpar(valor);
    }
  ...
  try {
    const mapeados = dados.map(d => ({
      ...
      nome: get(d, ['Colaborador', 'COLABORADOR', 'Nome', 'NOME', 'nome', 'Funcionário', 'Funcionario']),
      user_jira: get(d, ['User Jira', 'USER JIRA']),
      user_blip: get(d, ['User Blip', 'USER BLIP']),
      email: get(d, ['Email', 'E-mail', 'EMAIL']),
      ...
      admissao: dataBR(get(d, ['Admissão', 'Admissao', 'ADMISSÃO'])),
      cargo: get(d, ['Cargo', 'CARGO']),
      cpf: get(d, ['Cpf', 'CPF']),
      data_nasc: dataBR(get(d, ['Data Nasc', 'Data Nascimento', 'DATA NASC'])),
      sexo: get(d, ['Sexo', 'SEXO']),
      filial: get(d, ['Filial', 'FILIAL']),
      area: get(d, ['Área', 'Area', 'ÁREA']),
      telefone: get(d, ['Telefone', 'TELEFONE']),
      primeiro_dia_ferias: dataBR(get(d, ['Primeiro dia férias', 'Primeiro dia de férias', 'Primeiro Dia Férias'])),
      ultimo_dia_ferias: dataBR(get(d, ['Último dia férias', 'Ultimo dia ferias', 'Último Dia Férias'])),

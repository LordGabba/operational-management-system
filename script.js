...
function parsearCSV(texto) {
  ...
  const headers = dividir(linhas[0]);
...
async async function confirmarImportacao(dados) {
  ...
  const normalizarChave = v => String(v ?? '')
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
    .replace(/[^a-z0-9]/g, '');
  ...
  try {
    const mapeados = dados.map(d => ({
      matricula: get(d, ['matricula', 'Matrícula', 'MATRICULA', 'MATRÍCULA']) || null,
      ...
      ultimo_dia_ferias: data(get(d, ['Último dia férias', 'Ultimo dia ferias', 'Último dia de férias'])),

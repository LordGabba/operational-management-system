    ...
    try {
      ...
      if (ext === 'csv') {
        ...
        if (typeof XLSX === 'undefined') {
          toast('Biblioteca XLSX não carregada. Exporte a planilha como CSV.', 'error');
          return;
        }
...
async function confirmarImportacao(dados) {
  ...
  const mapaCampos = {
    ...
    nome: ['colaborador', 'nome', 'funcionario', 'funcionarios'],
    user_jira: ['userjira', 'jira'],
    user_blip: ['userblip', 'blip'],
    email: ['email'],
    reporte: ['reporte'],
    status: ['status'],
    celula: ['celula'],
    ...
    escala: ['escala'],
    saida: ['saida'],
    admissao: ['admissao', 'dataadmissao'],
    cargo: ['cargo'],
    cpf: ['cpf'],
    data_nasc: ['datanasc', 'datanascimento', 'nascimento'],
    idade: ['idade'],
    sexo: ['sexo'],
    filial: ['filial'],
    area: ['area'],
    telefone: ['telefone', 'celular'],
    primeiro_dia_ferias: ['primeirodiaferias', 'primeirodiadeferias'],
    ultimo_dia_ferias: ['ultimodiaferias', 'ultimodiadeferias'],
    turno: ['turno'],
    supervisor: ['supervisor'],
    coordenador: ['coordenador'],
    banco_horas: ['bancodehoras', 'bancohoras'],
  ...
  const prepararLinha = linha => {
    ...
    Object.keys(linha).forEach(chaveOriginal => {
      normalizada[normalizar(chaveOriginal)] = linha[chaveOriginal];
    });
    return normalizada;
  };

  const pegar = (linhaNormalizada, nomes) => {
    for (const nome of nomes) {
      const valor = linhaNormalizada[nome];
  ...
  try {
    const mapeados = dados.map(linha => {
      const l = prepararLinha(linha);

      return {
        matricula: pegar(l, mapaCampos.matricula) || null,
        centro_custo: pegar(l, mapaCampos.centro_custo) || null,
        nome: pegar(l, mapaCampos.nome),
        user_jira: pegar(l, mapaCampos.user_jira) || null,
        user_blip: pegar(l, mapaCampos.user_blip) || null,
        email: pegar(l, mapaCampos.email) || null,
        reporte: pegar(l, mapaCampos.reporte) || null,
        status: pegar(l, mapaCampos.status) || 'Ativo',
        celula: pegar(l, mapaCampos.celula) || null,
        grupo: pegar(l, mapaCampos.grupo) || null,
        tipo: pegar(l, mapaCampos.tipo) || null,
        horario: pegar(l, mapaCampos.horario) || null,
        escala: pegar(l, mapaCampos.escala) || null,
        saida: pegar(l, mapaCampos.saida) || null,
        admissao: data(pegar(l, mapaCampos.admissao)),
        cargo: pegar(l, mapaCampos.cargo) || null,
        cpf: pegar(l, mapaCampos.cpf) || null,
        data_nasc: data(pegar(l, mapaCampos.data_nasc)),
        idade: numero(pegar(l, mapaCampos.idade)),
        sexo: pegar(l, mapaCampos.sexo) || null,
        filial: pegar(l, mapaCampos.filial) || null,
        area: pegar(l, mapaCampos.area) || null,
        telefone: pegar(l, mapaCampos.telefone) || null,
        primeiro_dia_ferias: data(pegar(l, mapaCampos.primeiro_dia_ferias)),
        ultimo_dia_ferias: data(pegar(l, mapaCampos.ultimo_dia_ferias)),
        turno: pegar(l, mapaCampos.turno) || null,
        supervisor: pegar(l, mapaCampos.supervisor) || null,
        coordenador: pegar(l, mapaCampos.coordenador) || null,
        banco_horas: numero(pegar(l, mapaCampos.banco_horas)),
        jornada_semanal: numero(pegar(l, mapaCampos.jornada_semanal)) || 44,
        observacoes: pegar(l, mapaCampos.observacoes) || null
      };
    }).filter(d => d.nome);

    if (!mapeados.length) {
      const colunas = Object.keys(dados[0] || {}).join(', ');
      toast('Nenhum registro válido. Não encontrei a coluna Colaborador/Nome. Colunas lidas: ' + colunas, 'error');
      return;
    }
    ...
    if (destino === 'colaboradores') {
      ...
      const { error } = await db.from('staff').upsert(mapeados, { onConflict: 'matricula' });

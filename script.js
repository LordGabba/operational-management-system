    ...
    try {
      ...
      if (ext === 'csv') {
        ...
        if (typeof XLSX !== 'undefined') {
          ...
          toast('Biblioteca XLSX não carregada. Exporte como CSV ou adicione SheetJS.', 'warning');
          return;
        }
      }
...
function parsearCSV(texto) {
  ...
  const headers = primeiraLinha.split(separador).map(limparCelula);

  return linhas.slice(1).map(linha => {
    const valores = linha.split(separador).map(limparCelula);
    const obj = {};
    headers.forEach((h, i) => {
      obj[h] = valores[i] ?? '';
    });
...
async function confirmarImportacao(dados) {
  ...
  const get = (obj, nomesPossiveis) => {
    const mapa = {};
    Object.keys(obj).forEach(k => {
      mapa[normalizar(k)] = obj[k];
    });
  ...
  try {
    const mapeados = dados.map(d => ({
      matricula: get(d, ['matricula', 'matrícula']),
      centro_custo: get(d, ['centro de custo', 'centro_custo']),
      nome: get(d, ['colaborador', 'nome', 'funcionario', 'funcionário']),
      user_jira: get(d, ['user jira', 'jira']),
      user_blip: get(d, ['user blip', 'blip']),
      email: get(d, ['email', 'e-mail']),
      reporte: get(d, ['reporte']),
      status: get(d, ['status']) || 'Ativo',
      celula: get(d, ['celula', 'célula']),
      grupo: get(d, ['grupo']),
      tipo: get(d, ['tipo']),
      horario: get(d, ['horario', 'horário']),
      escala: get(d, ['escala']),
      saida: get(d, ['saida', 'saída']),
      admissao: data(get(d, ['admissao', 'admissão'])),
      cargo: get(d, ['cargo']),
      cpf: get(d, ['cpf']),
      data_nasc: data(get(d, ['data nasc', 'data nascimento', 'nascimento'])),
      sexo: get(d, ['sexo']),
      filial: get(d, ['filial']),
      area: get(d, ['area', 'área']),
      telefone: get(d, ['telefone']),
      primeiro_dia_ferias: data(get(d, ['primeiro dia ferias', 'primeiro dia férias', 'primeiro dia de ferias', 'primeiro dia de férias'])),
      ultimo_dia_ferias: data(get(d, ['ultimo dia ferias', 'último dia férias', 'ultimo dia de ferias', 'último dia de férias'])),
      turno: get(d, ['turno']),
      supervisor: get(d, ['supervisor']),
      coordenador: get(d, ['coordenador']),
      banco_horas: numero(get(d, ['banco de horas', 'banco horas'])),
      jornada_semanal: numero(get(d, ['jornada semanal', 'jornada semanal h'])) || 44,
      observacoes: get(d, ['observacoes', 'observações'])
    })).filter(d => d.nome);

    if (!mapeados.length) {
      const colunas = Object.keys(dados[0] || {}).join('; ');
      toast('Nenhum registro válido. Não encontrei coluna de nome/colaborador. Colunas lidas: ' + colunas, 'error');
      return;
    }

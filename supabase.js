...
function calcularCamposAuto(dados) {
  ...
  ['admissao', 'data_nasc', 'primeiro_dia_ferias', 'ultimo_dia_ferias', 'data_desligamento'].forEach(campo => {
    if (campo in d) d[campo] = normalizarData(d[campo]);
  });

  ['banco_horas', 'jornada_semanal'].forEach(campo => {
    if (campo in d) {
      const valor = String(d[campo] ?? '').trim().replace(',', '.');
      if (!valor) d[campo] = null;
      else {
        const n = Number(valor);
        d[campo] = Number.isFinite(n) ? n : null;
      }
    }
  });

  if (d.data_nasc) {
    const [ano, mes, dia] = d.data_nasc.split('-').map(Number);
  ...
  if (d.admissao) {
    const [ano, mes, dia] = d.admissao.split('-').map(Number);
